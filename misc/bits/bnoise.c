...
int main (int   argc, 
    ...
    if (argc !=  6)
        {
        printf ("Usage: bnoise <output m5b fname A> <output m5b fname B> <fs (MHz)> <dur (s)> <corr coeff>\n");
        printf ("e.g.:  bnoise filea.m5b fileb.m5b 32 5 0.1\n");
        return(1);
        }
    ...
    for (isec=0; isec<idur; isec++)
        ...
        for (ifr=0; ifr<nfps; ifr++)
            ...
            for (isamp=0; isamp<2500; isamp++)
                ...
                a = gsl_ran_gaussian (r, 1.0) + c;
                b = gsl_ran_gaussian (r, 1.0) + c;
                                    // re-sample to 2 bits
                ...
                adata[isamp+4] = a2 | a2 << 2;
                bdata[isamp+4] = b2 | b2 << 2;
                }
