...
void xcorr2(unsigned char *, unsigned char *, double *, double *, double, double, double,int,int,int,int,int);
...
void plot(double *,double *,int,int,char *,char *,char *,char *,int,int,double,int);
void dumpdata(char *, char *, char *, double *, double *, int);
...
static double pltamp[NFRQSER*NSIZ2], pltphs[NFRQSER*NSIZ2];


int process2(char *filename1, char *filename2, char *filenameout, char *filenamedata, int smode, double swidth, double fwidth,int rev,int tbit, double ampfac, int verb)
{
 ...
 for(ichan=0;ichan<NCHAN;ichan++){
	...
	readcount=0;
	  if (ist == 0 && get0){
	    fread(dd, sizeof(dd), 1, file1);
//	    printf("dd %x %x %x\n",dd[0],dd[1],dd[2]);
	    readcount++;
	  }
	  if (ist ==1 && get1){
            fread(dd, sizeof(dd), 1, file2);
	    readcount++;
	  }
          if(readcount){
            if ((ist == 0 && get0) || (ist == 1 && get1)) {
                  if (verb>1) {
                      ...
                      printf("%d: %s ", ist, ist ? filename2 : filename1);
                  }
		  // get the frequency
		  sync=dd[0];
		  frame = dd[1]&0x7fff;
...
//	    delay=10.0;
		...
		for(j=0;j<NCHAN;j++)
            ...
            xcorr2(vblock, vblock2, xspecr, xspeca, delay, phase, fwidth,num,nbins,ichan,tbit,verb);
            num++;
            if (num == smode) {
                xcalc(xspecr, xspeca, num, secs, swidth, fwidth, nbins,ichan);
	       if(ichan==NCHAN-1) plot(xspecr, xspeca, num, NCHAN,
                    filename1,filename2,filenameout,filenamedata,
                    rev,tbit, ampfac,verb);
                num = 0;
		iiseg=0;
            }
	 }
...
void xcorr2(unsigned char vblock[], unsigned char vblock2[], double xspecr[], double xspeca[], double dsteer, double phsteer,double fwidth, int num, int nbins, int chan, int tbit, int verb)
...
// if(sft==64)    printf("i %d m %d ch ch1 ch2 %x %x %x da %x %x\n",i,m,ch,ch1,ch2,da[i+2],da2[m]);
   ...
   if(sft > NLAG/2 - 5 && sft < NLAG/2 + 5 && verb>1)
      printf("xcorr sft %d %f\n",sft,aut[sft]);
    }
...
// printf("i %3d amp %10.4f\n",i,sqrt(px[i]*px[i]+py[i]*py[i]));
    ...
    printf("%10.6f amp %7.4f rate %8.3f delay %4.0f ns "
           "k %3d fwidth %f nbins %d ch %d\n",
           secs,amp,maxrate,delay,k,fwidth,nbins,chan);
}

void plot(double xspecr[], double xspeca[], int num, int chan,char *filename1,char *filename2,char *filenameout, char *filenamedata, int rev,int tbit, double ampfac, int verb)
{
    ...
    FILE *file3;
       ...
       if ((file3 = fopen(filenameout, "w")) == NULL) {
            printf(" ps output error\n");
            return;
       }
	...
	amp = amp/num;
        if (*filenamedata && iter==0) {
            pltamp[ichan*NSIZ2+i] = amp;
            pltphs[ichan*NSIZ2+i] = phase;
        }
        avamp += amp;
        namp++;
        xx2=x+(i+ichan*NSIZ2)*500.0/(NSIZ2*16.0);
        if(iter==0) yy2 = y+amp*75.0*ampfac;
	...
	fprintf(file3,"30 300 moveto\n 90 rotate\n (phase) show\n -90 rotate\n");
        ...
        if (*filenamedata)
          dumpdata(filenamedata, filename1, filename2, pltamp, pltphs, chan);
}

void dumpdata(char *filenamedata, char *f1, char *f2,
              double amp[], double phs[], int chan)
{
    FILE *file4;
    int ch, ii;

    if ((file4 = fopen(filenamedata, "w")) == NULL) {
        printf(" data output error\n");
        return;
    }
    fprintf(file4, "#\n# Correlation of %s v %s\n#\n", f1, f2);

    fprintf(file4, "# Amplitudes\n");
    for (ii = 0; ii < NSIZ2; ii++) {
        fprintf(file4, "%4d", ii);
        for (ch = 0; ch < chan; ch++)
            fprintf(file4, " %e", amp[ch*NSIZ2 + ii]);
        fprintf(file4, "\n");
    }
    fprintf(file4, "\n\n");

    fprintf(file4, "# Phases\n");
    for (ii = 0; ii < NSIZ2; ii++) {
        fprintf(file4, "%4d", ii);
        for (ch = 0; ch < chan; ch++)
            fprintf(file4, " %e", phs[ch*NSIZ2 + ii]);
        fprintf(file4, "\n");
    }
    fprintf(file4, "\n\n");

    fprintf(file4, "# Amp & Phase with channels joined\n");

    for (ch = 0; ch < chan; ch++) {
        for (ii = 0; ii < NSIZ2; ii++) {
            fprintf(file4, "%6d %e %e\n",
                ch*NSIZ2 + ii, amp[ch*NSIZ2 + ii], phs[ch*NSIZ2 + ii]);
        }
    }
    fprintf(file4, "\n\n");
