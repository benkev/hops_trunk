...
where the required options are
  ...
  Version $VERSION
  v0.03: Added pc_phases info (note to user: edit output to append _l and so on)
...
my $channames = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ123456789";
...
for ($k=0;$k<=$#baselineset;$k++){
	...
	print "$baselineset[$k]: ";
	my $printchans = "";
        if ($nchan < length($channames)){
	    $printchans = substr($channames,0,$nchan);
	}
	print "pc_phases $printchans ";
	for ($j=0;$j<$nchan;$j++){
