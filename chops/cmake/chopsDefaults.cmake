...
macro(chops_install_headers)
    install(FILES ${ARGN} DESTINATION ${INCLUDE_INSTALL_DIR})
endmacro()

macro(chops_install_libraries)
    install(TARGETS ${ARGN} EXPORT chopsTargets DESTINATION ${LIB_INSTALL_DIR})
...
macro(chops_require_clang_version VERSION)
...
macro(chops_require_xcode_version VERSION)
...
macro(chops_require_gcc_version VERSION)
    if(COMPILER_IS_GNU)
        if(COMPILER_VERSION VERSION_LESS ${VERSION})
...
macro( chops_include_directories )
    ...
    foreach(dir ${ARGN})
        ...
        if(pos LESS 0)
            ...
            list(APPEND chops_DIRS ${dir})
        endif()
    endforeach()

    include_directories( ${chops_DIRS} )
    include_directories( SYSTEM;${SYSTEM_DIRS} )
endmacro()

macro( chops_internal_include_directories )
    ...
    chops_include_directories( ${MODULE_HEADER_DIRS} )

endmacro()


macro( chops_external_include_directories )
    ...
    chops_include_directories( ${ARGN} )

endmacro()


macro (chops_configure_automake RELPATH_LIST LIBLIST LIBNAME HEADER_LIST SOURCE_LIST)
    if (CHOPS_RECONFIGURE_AUTOMAKE)
        ...
        set(MAKEFILE_AM_IN "${CMAKE_CURRENT_SOURCE_DIR}/Makefile.am.cmake")
...
macro (chops_configure_exe_automake RELPATH_LIST INTERNAL_LIBLIST EXENAME HEADER_LIST SOURCE_LIST)
    if (CHOPS_RECONFIGURE_AUTOMAKE)
        #construct the list of include paths
        list(APPEND AM_INCPATH_LIST "-I$(srcdir)/include")
        foreach(rpath ${RELPATH_LIST})
