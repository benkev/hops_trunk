...
class Configuration(report_lib.JsonSerializableObject ):
    ...
    def __init__(self):
        ...
        self.network_reference_station = '' #station to be used as network phase reference (not strictly needed, except in mixed-mode)
        self.target_stations = '' #list of VGOS stations to generate phase/delay offsets for
...
class MixedModeFourphaseSingleScanProcessor(object):
    ...
    def set_stations(self, legacy_stations='', target_stations=''):
        self.legacy_stations = legacy_stations.strip()
        self.target_stations = target_stations.strip()

    def calculate_delay_phase_offsets(self):
        """ compute the phase/delay offsets for Y-X at each station """

        leg_st = self.legacy_stations
        vgos_st = self.target_stations

        set_commands = "set gen_cf_record true"
        blpp_collections = []

        bl_pp_dict = processing.construct_valid_mixedmode_baseline_polproduct_dict(self.scan_dir, self.legacy_stations, self.target_stations)
        for bl in bl_pp_dict.keys():
            pp_list = bl_pp_dict[bl]
            legacy_is_ref = True
            lst = bl[0]
            vst = bl[1]
            pol_products = ['RX', 'RY']
            if bl[0] not in self.legacy_stations:
                legacy_is_ref = False
                lst = bl[1]
                vst = bl[0]
                pol_products = ['XR', 'YR']
            self.bl_pol_products[bl] = pol_products
            temp_blpp_collections = processing.load_batch_cut_and_sort_mixedmode( \
                self.exp_dir, lst, vst, self.control_file_path, \
                set_commands, min_snr=self.min_snr, network_reference_baselines_only=False, num_processes=self.num_proc, \
                start_scan_limit=self.scan_name, stop_scan_limit=self.scan_name, \
                only_complete=False, pol_products=pol_products, frequency_group=self.frequency_group, use_progress_ticker=self.use_progress_ticker \
            )
            blpp_collections.extend(temp_blpp_collections)

        #loop over station-pol-product-baseline collections, extracting mbd and phase
        results_table = dict()
        printable_results_table = []
        valid_stations = set()
        valid_baselines = set()
        for blc in blpp_collections:
            bl = blc.baseline
            if blc.get_min_snr() > self.min_snr:
                #SNR ok, include this baseline
                valid_baselines.add(bl)
                valid_stations.add(bl[0])
                valid_stations.add(bl[1])
                for pp in self.bl_pol_products[bl]:
                    ff = blc.get_fringe_object(pp)
                    if ff is not None:
                        results = ( ff.snr, ff.sbdelay, ff.mbdelay, ff.resid_phas, ff.dtec, ff.amp*10000 )
                        blpp_key  = (bl, pp)
                        results_table[blpp_key] = results
                        printable_results_table.append( [bl, pp, ff.snr, ff.sbdelay, ff.mbdelay, ff.resid_phas, ff.dtec, ff.amp*10000] )
            else:
                #not enough cross power in this scan, so discard this baseline and issue warning
                self.error_condition = 2
                fourphase_logger.debug( '\n**** ERROR **** ' + bl + ' has min snr of only ' +  str(blc.get_min_snr() ) + ' and mean snr for all other pol-products is ' + str(blc.get_mean_snr()) )
        ...
        headers=["bl", "pp", "snr", "sbd", "mbd", "ph", "dtec", "amp"]
        print(utility.tabulate(printable_results_table, headers, n_digits=4, padding=7))
        ...
        for bl in valid_baselines:
            ...
            if (bl,'YR') in results_table and (bl, 'XR') in results_table:
                ...
                pyr_xr = utility.limit_periodic_quantity_to_range(results_table[(bl,'YR')][3] - results_table[(bl,'XR')][3], low_value=-180.0, high_value=180.0)
                self.station_y_minus_x_delays[bl[0]].append( ( dyr_xr, bl[1] ) )
                self.station_y_minus_x_phases[bl[0]].append( ( pyr_xr, bl[1] ) )
            if (bl,'RY') in results_table and (bl, 'RX') in results_table:
                #flip sign when taking phase/delay difference, if the legacy-station is the reference station on this baseline
                dry_rx =  -1.0*( results_table[(bl,'RY')][2] - results_table[(bl,'RX')][2] )
                pry_rx = utility.limit_periodic_quantity_to_range( -1.0*( results_table[(bl,'RY')][3] - results_table[(bl,'RX')][3] ), low_value=-180.0, high_value=180.0)
                self.station_y_minus_x_delays[bl[1]].append( ( dry_rx, bl[0] ) )
                self.station_y_minus_x_phases[bl[1]].append( ( pry_rx, bl[0] ) )
...
class ExperimentReportData( report_lib.JsonSerializableObject ):
    ...
    def update_config(self, configuration_obj):
        ...
        self.config_obj.network_reference_station = configuration_obj.network_reference_station
        self.config_obj.target_stations = configuration_obj.target_stations
...
def generate_station_phase_delay_corrections(config_obj, report_data_obj=None):
    ...
    all_stations = config_obj.network_reference_station + target_stations
    ...
    if config_obj.mode == 'VGOS':
        ...
        only_complete_blc = False

    fourphase_logger.info("generating Y-X delay/phase corrections from data in exp_dir: " + exp_dir + " for stations: " + all_stations)
    ...
    baseline_indexed_scan_lists = select_fourphase_scans_by_baseline(scan_blc, exp_dir, all_stations, dtec_tolerance, min_snr, dtec_nom, max_num_scans)
    ...
    if config_obj.mode == 'VGOS':
        station_delay_phase_results = { st:SingleStationPhaseDelayOffsets(st) for st in all_stations }
    elif config_obj.mode == 'MIXED_MODE':
        station_delay_phase_results = { st:SingleStationPhaseDelayOffsets(st) for st in target_stations }
    ...
    for root in root_file_set:
        if config_obj.mode == 'VGOS':
            ...
            scan_processor.frequency_group = config_obj.frequency_group
            scan_processor.set_stations(legacy_stations=config_obj.network_reference_station, target_stations=config_obj.target_stations)
            scan_processor.calculate_delay_phase_offsets()
            station_delays = scan_processor.get_raw_station_delay_offsets()
            station_phases = scan_processor.get_raw_station_phase_offsets()
            if scan_processor.error_condition == 0:
...
def construct_control_file_lines(config_obj, station_delay_phase_results):
    ...
    for st in all_stations:
        ...
        if result.get_n_total_entries() >= 1 and result.get_n_used_entries() >= 1:
            ...
            if config_obj.mode == "MIXED_MODE":
                cf_lines += '\nif station ' + st + ' and f_group ' + config_obj.frequency_group + "\n"
            else:
                cf_lines += '\nif station ' + st + '\n'
...
def generate_fourphase_control_file(config_obj, output_control_filename):
    ...
    cf_lines = construct_control_file_lines(config_obj, station_delay_phase_results)
