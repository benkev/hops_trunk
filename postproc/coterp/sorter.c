/************************************************************************/
/*									*/
/* This routine sorts data read by coterp.  A shell			*/
...
/*									*/
/* Created November 29 1995 by CJL, stolen from cofit			*/
/*									*/
/************************************************************************/
...
int nrec;
	...
	data[data[i].order].lastorder = i;
					...
					/* and passes required */
    interval = 1; npass = 0;
    while ((interval = 3*interval + 1) <= nrec) npass++;
    interval = interval/3;
