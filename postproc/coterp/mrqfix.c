...
int mrqfix(x,y,sig,ndata,a,ia,ma,covar,alpha,chisq,funcs,alamda)
double **alpha,**covar,*alamda,*chisq,a[],sig[],x[],y[];
int ia[],ma,ndata;
void (*funcs)();
{
  void covsrt(),mrqcof(),svbksb();
  int dsvdcmpp();
  int j,k,l,m,decomp,go;
  double wmax,wmin,**u,**v,*w,*tempin,*tempsol;
  static int mfit;
 ...
 /*printf("right before matrix and vector setting\n"); */ 
	u=dmatrix(1,mfit,1,mfit);
	v=dmatrix(1,mfit,1,mfit);
	w=dvector(1,mfit);
        tempin=dvector(1,mfit);
        tempsol=dvector(1,mfit);
...
/*   If unsuccessful then return -2 */
        decomp = 0;
	decomp=dsvdcmpp(u,mfit,mfit,w,v);
...
/* printf("right after first backsub\n"); */
	...
	free_dmatrix(u,1,mfit,1,mfit);
	free_dmatrix(v,1,mfit,1,mfit);
	free_dvector(w,1,mfit);
	free_dvector(tempsol,1,mfit);
 	free_dvector(tempin,1,mfit); 
  ...
  if (*alamda == 0.0) {
    ...
    return(0);	/* presuming this to be a valid return case */
  }
  for (j=0,l=1;l<=ma;l++)
    if (ia[l]) atry[l]=a[l]+da[++j];
  go = 1;
  for (l=1;l<=ma;l++) if (atry[l]<=0.0) go = -1;
