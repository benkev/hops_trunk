...
#define DIRC "/home/jball/C/" /* Where to find the *.c and *.h files */ 
/* *** */ 
#endif /* MARK5DIR_ORIG */
#define MAXSCANS      1024 /* Maximum number of scans in SDirO */ 
#define LRGMAXSCANS  65536 /* Maximum number of scans in SDir */ 
...
#endif /* MARK5DIR_ORIG */
struct SDirO { /* Older directory of recorded scans, etc. */ 
  ...
  /* (sizeof(struct SDirO) must be less than XLR_MAX_UDIR_LENGTH) */ 
  }; /* End of struct SDirO */ 
#if MARK5DIR_ORIG
extern volatile struct SDirO * sdirio; /* Older, smaller version */ 
#endif /* MARK5DIR_ORIG */
struct SDirL { /* Newer, larger directory of recorded scans, etc. */
  int nscans; /* Number of scans herein */
  int n; /* Next scan to be accessed by "next_scan" */
  char scanName[LRGMAXSCANS][MAXLENGTH]; /* Extended name */
  unsigned long long start[LRGMAXSCANS]; /* Start byte position */
  unsigned long long length[LRGMAXSCANS]; /* Length in bytes */
  unsigned long long recpnt; /* Record offset, bytes (not a pointer) */
  long long plapnt; /* Play offset, bytes */
      /* Beware:  Some folks call these indices "pointers" */
  double playRate; /* Playback clock rate, MHz */
...
#if MARK5DIR_ORIG
extern volatile struct SDir * sdir; /* Newer, larger version */ 
...
struct LDir { /* Directory of last scans and file names, etc. */ 
  ...
  int szlarge; /* TRUE if we read (!) the new very-large user directory */ 
  }; /* End of struct LDir */ 
...
int writedir(int noss); /* Write directory to file and also to SS disks 
                         * unless noss is TRUE */ 
